...
public class RegistryConfig implements IConfigCheck {
    ...
    private CuratorFramework create(String connectString, Integer sessionTimeout, String namespace, int retry) {
        ...
        return builder.connectString(connectString).sessionTimeoutMs(sessionTimeout).connectionTimeoutMs(sessionTimeout).namespace(namespace)
                .retryPolicy(new ExponentialBackoffRetry(1000,retry)).defaultData(null).build();
    }
