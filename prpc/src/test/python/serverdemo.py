...
from jdd.prpc import server         # 导入prpc 的server
from jdd.prpc.common import config  # 导入prpc 的配置库
...
#定义回调函数， prpc 采用的是多进程方式，此函数在创建进程的时候会进行调用， 具体可以参考thrift python 多进程server
def callback():
